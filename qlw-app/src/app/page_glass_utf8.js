        ...
        <div className="hero-split-left" style={{ alignItems: 'center', textAlign: 'left', padding: '0 5% 0 8%' }}>
          <div className="hero-split-left-content" style={{ background: 'transparent', padding: '0', boxShadow: 'none', backdropFilter: 'none', maxWidth: '700px', width: '100%' }}>
            ...
            <p style={{ color: 'white', fontWeight: '600', fontSize: '0.95rem', marginBottom: '15px', wordSpacing: '2px' }}>
              Retirement Planning <span style={{ color: '#e8c678', margin: '0 5px' }}>ΓÇó</span> Wealth Strategies <span style={{ color: '#e8c678', margin: '0 5px' }}>ΓÇó</span> Financial Education
            </p>
            ...
            <div style={{ display: 'flex', gap: '15px', marginBottom: '30px', flexWrap: 'wrap' }}>
              <a href="#" onClick={(e) => {
                ...
                <span style={{ fontSize: '1.1rem' }}>≡ƒôà</span> BOOK DISCOVERY CALL
              </a>
              <a href="/who_we_are/about" className="btn btn-primary" style={{ background: 'transparent', color: 'white', border: '2px solid rgba(255,255,255,0.3)', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '10px', padding: '12px 24px', fontSize: '0.85rem' }}>
                <span style={{ fontSize: '1.1rem', color: '#e8c678' }}>Γû╖</span> EXPLORE OUR SERVICES
              </a>
            </div>
      ...
      <section className="why-choose-section">
        <div className="container" style={{ maxWidth: '1000px' }}>
          ...
          <div className="why-choose-grid">
            <WhyChooseItem icon="≡ƒÄ»" title="Expert Guidance" desc="Licensed professionals providing personalized financial strategies tailored to your goals." />
            <WhyChooseItem icon="≡ƒôï" title="Comprehensive Solutions" desc="From insurance to real estate - all your financial needs under one trusted roof." />
            <WhyChooseItem icon="≡ƒôê" title="Proven Results" desc="Trusted by hundreds of families to build, protect, and grow their wealth for generations." />
          </div>
        </div>
      </section>
              ...
              <div className="opp-card">
                ...
                <form id="contact-form" className="contact-form-grid" onSubmit={handleFormSubmit}>
                  ...
                  <div className="grid-span-2">
                    <button
                      ...
                      style={{ marginTop: 0 }}
                    >
                      {isSubmitting ? "Sending..." : "Send Message ΓåÆ"}
                    </button>
                  </div>
                </form>
                {error && (
                ...
                {formSubmitted && (
                  <div className="form-success" id="form-success" style={{ display: 'block', background: 'linear-gradient(135deg, rgba(34, 197, 94, 0.1), rgba(34, 197, 94, 0.05))', border: '1px solid rgba(34, 197, 94, 0.3)', borderRadius: '10px', padding: '20px', textAlign: 'center', color: '#166534', fontWeight: '600', marginTop: '15px' }}>
                    Γ£à Thank you! Your message has been sent. We&apos;ll reach out within 24 hours.
                  </div>
                )}
              </div>
            </div>
    ...
    <a href={href} className={`flip-card reveal reveal-delay-${delay}`} style={{ textDecoration: 'none' }}>
      <div className="flip-card-inner">
        ...
        <div className="flip-card-back">
          ...
          <p>{desc}</p>
          <span className="card-arrow">Learn more ΓåÆ</span>
        </div>
      </div>
    </a>
  );
      ...
      <div className="event-card">
        ...
        <div className="event-card-body">
          <span className={`event-badge-status ${isPast ? 'badge-completed' : 'badge-upcoming'}`}>
            {isPast ? 'Γ£ô Completed' : 'ΓùÅ Upcoming'}
          </span>
          <p>{event.description}</p>
          <div className="event-meta">
            {isPast ? (
              <><span>≡ƒæÑ {event.attendees || 'N/A'} Attendees</span><span>≡ƒôì {event.location}</span></>
            ) : (
              <><span>≡ƒòÉ {event.time}</span><span>≡ƒôì {event.location}</span><span>≡ƒÄƒ∩╕Å {event.price || 'Free'}</span></>
            )}
          </div>
          {isPast ? (
            <a href={event.recapLink || '#'} className="event-btn event-btn-recap">View Recap ΓåÆ</a>
          ) : (
...
function OppPerk({ text }) {
  return (
    <div className="opp-perk"><div className="opp-perk-icon">Γ£ô</div>{text}</div>
  );
}
